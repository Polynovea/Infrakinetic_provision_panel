...
**1A.3 — Governance DB.** Corrected architecture (2026-09-11): Governance shares the SAME existing production Postgres database as Infrakinetic — isolation is a dedicated `governance` schema owned by a dedicated `governance_app` role (`NOSUPERUSER`/`NOCREATEDB`/`NOCREATEROLE`/`NOBYPASSRLS`), not a separate database. Migrations (`0001_operator_identity_schema.sql` + `0002_governance_db_foundation.sql`, every object schema-qualified as `governance.*`), a lazily-configured DB connection layer, a migration runner (`npm run db:migrate`), and Postgres-backed `OperatorDirectory`/`OperatorSessionStore` adapters (replacing 1A.2's in-memory ones, per that subphase's own design) are built and locally verified (76/76 tests) against pg-mem. **Nothing has been provisioned against the real database** — this session has neither AWS credentials nor a local Postgres/Docker install; the provisioning script (`backend/provisioning/001_create_role_and_schema.sql`) is authored but has never been run. See `docs/1A.3_status.md`.
