...
**1A.2 — Privileged operator identity.** Application-side identity/session/role/scope model, `requireManagementApiAuth` + `authorize.ts` authorization middleware, Cognito JWT verification boundary (env-configured, no hardcoded AWS identifiers), the operator-identity persistence schema, and an idempotent, audited operator-bootstrap CLI (`npm run bootstrap:operator`) are built and locally verified (37/37 tests). Corrected finding (2026-09-11): this backend is designed to **reuse Infrakinetic's existing Cognito pool via a new, dedicated app client**, not stand up a second pool — see `docs/1A.2_status.md` "2026-09-11 follow-up" for the evidence and the one open question (pool-wide MFA posture) that needs live AWS access to close. Live Cognito integration and the real internal-Polynovea-tenant/operator bootstrap remain **pending** — this session has neither AWS credentials nor database connectivity, a capability gap the status doc documents precisely.
