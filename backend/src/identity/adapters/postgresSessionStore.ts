...
// 1A.3 — Postgres-backed OperatorSessionStore, against
// migrations/0001_operator_identity_schema.sql (see that file's header for
// why operator_id/issued_at/expires_at are nullable: the
// OperatorSessionStore port never carries them, because 1A.2 does not mint
// a separate management assertion with its own issuance event — "session"
// is just the verified token's own jti, and revoke()/recordStepUp() may be
// the first write ever made for a given session id). Table references are
// schema-qualified (`governance.*`) per the 2026-09-11 architecture
// correction — see docs/1A.3_status.md.
//
...
export class PostgresSessionStore implements OperatorSessionStore {
  ...
  async isRevoked(sessionId: string): Promise<boolean> {
    const result = await this.db.query<SessionRow>(
      "SELECT revoked_at, step_up_at, step_up_method FROM governance.operator_sessions WHERE session_id = $1",
      [sessionId],
    );
  ...
  async recordStepUp(sessionId: string, state: StepUpState): Promise<void> {
    await this.db.query(
      `INSERT INTO governance.operator_sessions (session_id, step_up_at, step_up_method)
       VALUES ($1, $2, $3)
       ON CONFLICT (session_id) DO UPDATE
