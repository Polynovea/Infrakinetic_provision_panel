...
// there is no Governance database yet (1A.3). pg-mem is an in-memory PG
// emulator; this is schema verification, not proof the migration has been
...
describe("migrations/0001_operator_identity_schema.sql", () => {
  ...
  it("accepts a valid operator row and rejects an unknown status", () => {
    ...
    `);
    const rows = db.public.many("SELECT * FROM operators");
  ...
  it("cascades operator deletion into roles/scopes/sessions", () => {
    ...
    db.public.none("DELETE FROM operators WHERE operator_id = '88888888-8888-4888-8888-888888888888'");

    expect(db.public.many("SELECT * FROM operator_roles")).toHaveLength(0);
    expect(db.public.many("SELECT * FROM operator_sessions")).toHaveLength(0);
  });
});
